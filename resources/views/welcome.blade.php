...
<html lang="tr">
    ...
    <body class="antialiased">
        <div class="relative flex items-top min-h-screen sm:items-center py-4 sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 pb-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="text-sm text-gray-700 underline">Dashboard</a>
                    @else
                        <div class="grid grid-cols-2">
                            <div class="ml-50 mr-20 text-lg text-center fw-bold">
                                <img src="frontend/_images/logo.png" width="500" height="350">
                                <h3>"Btü öğrencilerinin buluşma noktası."</h3>
                            </div>
                            <div class="ml-40 mt-5">
                                <a href="{{ route('login') }}" class="rounded btn1 fw-bold text-xl">Giriş Yap</a>&nbsp;&nbsp;

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="rounded btn2 fw-bold text-xl">Kayıt Ol</a>
                                @endif
                            </div>
                        </div>
                    @endauth
            ...
            <div class="justify-center mt-20 max-w-6xl w-6xl mx-auto  sm:px-6 lg:px-8">
                <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg">
                    <div class="formbody grid grid-cols-1 md:grid-cols-2">
                        <div class="p-10">
                            <x-jet-validation-errors class="mb-4" />
                                ...
                                <form method="POST" action="{{ route('login') }}" class="ml-4 mr-4">
                                    ...
                                    <div class="mt-14 text-md">
                                        <x-jet-label for="email" value="{{ __('Email') }}" />
                                        <x-jet-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" placeholder="ogrencinumaraniz@ogrenci.btu.edu.tr" required autofocus />
                                    </div>

                                    <div class="mt-4 text-md">
                                        <x-jet-label for="password" value="{{ __('Şifre') }}" />
                                        <x-jet-input id="password" class="block mt-1 w-full" type="password" name="password" placeholder="xxxxxxxx" required autocomplete="current-password" />
                                    </div>

                                    <div class="block mt-4">
                                        <label for="remember_me" class="flex items-center">
                                            <x-jet-checkbox id="remember_me" name="remember" />
                                            <span class="ml-2 text-md text-gray-600">{{ __('Beni hatırla') }}</span>
                                        </label>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-end mt-4">
                                        ...
                                        <button class="rounded btn3 fw-bold text-light text-lg ml-4">
                                            {{ __('Giriş Yap') }}
                                        </button>
                                    </div>
                                </form>
                        ...
                        <div class="p-6 border-t border-gray-200 dark:border-gray-700 my-2 ml-4 mr-6 md:border-t-0">
                            <img src="frontend/_images/1.png" width="400" height="350" class="mx-auto">
                        </div>
                    </div>
                </div>
            </div>
